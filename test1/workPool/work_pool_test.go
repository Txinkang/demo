...
// 启动worker池,并为每个Worker分配一个ID,让每个Worker 进行工作
...
func (wm *WorkerManager) KeepLiveWorkers() {
	//如果有Worker已经死亡,则workChan会得到具体死亡的Worker,然后输出异常,最后重启
	for wk := range wm.workerChan {
		//log the error
		fmt.Printf("Worker %d stopped with err:[ % v]\n", wk.id, wk.err) //reset err
		wk.err = nil
		// 当前这个冰已经死亡了, 需要重新启动它的业务
		go wk.work(wm.workerChan)
	}
}
...
func (wk *worker) work(workerChan chan<- *worker) (err error) {
	//任何Goroutine只要异常退出或者正常退出都会调用defer函数,所以在defer中向WorkerManager的WorkChan发送通知
	defer func() {
		//捕获异常信息，防止panic直接退出
		if r := recover(); r != nil {
